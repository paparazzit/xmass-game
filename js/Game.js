...
function Game(opt) {
	...
	this.copyUrlBtn = document.querySelector("#copyUrl");
	this.logClicks = 0;
	this.allClicks = 0;
	...
	this.addToInventory = function (item) {
		if (this.successSound.paused) {
			...
			setTimeout(() => {
				...
				if (item.getAttribute("data-item") !== "hat") {
					thumb.append(item);
				} else {
					console.log(item.getAttribute("data-altimg"));
					item.setAttribute("src", item.getAttribute("data-altimg"));
					thumb.append(item);
				}
	...
	this.bodySwipe = function (ev) {
		if (this.gameStates.gameEnd) {
			console.log("GOTOVO");
			return;
		} else {
			ev.preventDefault();
		...
		console.log("bodySwipe", this.gameStates.gameEnd);
	};
	...
	this.swingStick = function (mob) {
		...
		if (this.beatingLog > 5) {
			...
			setTimeout(() => {
				...
				this.logSpeak(this.logMessages.finishTxt);
			}, 1800);
			if (!mob) {
				this.beatingUI.remove();
				this.removeItem();
	...
	this.logSpeak = function (msg) {
		...
		this.textBubble.children[1].innerHTML = logText;

		this.textBubble.classList.add("show");
		this.resetTimer();
	};
	...
	this.hideText = function () {
		this.textBubble.classList.remove("show");
		this.textBubble.children[1].innerHTML = "";
	};

	this.touchEvent = function () {
		console.log("TOUCH");

		this.body.addEventListener("touchmove", this.bodySwipe.bind(this), {
			passive: false,
		});
